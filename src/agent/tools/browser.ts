...
export const browserTools: Tool[] = [
  {
    name: 'browser',
    description: [
      'Stealth browser automation with anti-detection. Supports Playwright or Selenium backend.',
      'Actions: navigate|back|forward|refresh|screenshot|screenshot_full|screenshot_element|',
      'click|click_position|type|find|scroll|scroll_to|hover|double_click|right_click|drag|',
      'human_click|human_type|human_scroll|get_page|get_html|execute_js|save_html|',
      'fill_form|select|submit|find_forms|upload|wait_element|wait|cookies|',
      'local_storage_*|session_storage_*|tabs|new_tab|switch_tab|close_tab|',
      'get_interactive|get_aria_tree|click_text|find_text|google_search|duckduckgo_search|',
      'set_geolocation|print_to_pdf|emulate_device|inject_network_logger.',
      'human_type supports /enter /tab /escape /backspace inline key commands.',
      'For SPAs with randomized classes, prefer click_text/find_text over selectors.',
    ].join('\n'),
    parameters: {
      type: 'object',
      properties: {
