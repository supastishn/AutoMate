...
const memoryTool: Tool = {
  ...
  description: [
    ...
    'Actions: read|write|append (MEMORY.md), search (query), log (daily), files (list all),',
    'tier2_list/read/write/append/delete (topic files), archive_list/read/write/append/delete,',
    'identity_read/write (PERSONALITY.md etc), heartbeat_read/write, log_read, reindex, stats.',
    'Search supports: "exact phrase", -exclude, OR, file:name filters.',
  ].join('\n'),
  parameters: {
    type: 'object',
    properties: {
...
const bootstrapCompleteTool: Tool = {
  ...
  description: 'Mark first-run bootstrap as complete by deleting BOOTSTRAP.md. Call after initial setup when identity and user info are configured.',
