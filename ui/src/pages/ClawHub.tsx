...
export default function ClawHub() {
  ...
  const sevColor = (s: string) => s === 'high' ? colors.error : s === 'medium' ? colors.warning : colors.warning

  return (
    <div style={{ padding: 30, maxWidth: 1000 }}>
      <h1 style={{ fontSize: 24, marginBottom: 8, fontWeight: 600, color: colors.textPrimary }}>ClawHub Registry</h1>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20 }}>
        {(['browse', 'installed'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            padding: '8px 20px', background: tab === t ? colors.bgTertiary : colors.bgSecondary,
            color: tab === t ? colors.accent : colors.textSecondary, border: tab === t ? `1px solid ${colors.accent}` : `1px solid ${colors.borderLight}`,
      ...
      {preview && (
        <div style={{
          position: 'fixed', inset: 0, background: colors.bgOverlay, zIndex: 200,
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ fontSize: 16, margin: 0, color: colors.textPrimary }}>Preview: {preview.repo}</h3>
            ...
            <div style={{
              ...
              background: preview.vet.safe ? colors.bgSuccess : colors.bgDanger,
              border: `1px solid ${preview.vet.safe ? colors.borderSuccess : colors.borderDanger}`,
            }}>
              <div style={{
                fontSize: 14, fontWeight: 600, marginBottom: 8,
                color: preview.vet.safe ? colors.success : colors.error,
              }}>
                {preview.vet.safe
            ...
            {/* Content */}
            <div style={{
              background: colors.bgPrimary, padding: 12, borderRadius: 4, fontSize: 12,
              fontFamily: 'monospace', whiteSpace: 'pre-wrap', color: colors.textSecondary,
            ...
            <div style={{ marginTop: 16, display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setPreview(null)} style={{
                padding: '8px 16px', background: colors.border, color: colors.textSecondary, border: `1px solid ${colors.borderLight}`,
                borderRadius: 4, cursor: 'pointer', fontSize: 13,
              }}>Close</button>
              {preview.vet.safe && (
                <button onClick={() => { doInstall(preview.repo); setPreview(null) }} style={{
                  padding: '8px 16px', background: colors.accent, color: colors.accentContrast, border: 'none',
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
            <input
              ...
              style={{
                flex: 1, padding: '8px 12px', background: colors.bgHover, border: `1px solid ${colors.borderLight}`,
                borderRadius: 4, color: colors.textPrimary, fontSize: 13, outline: 'none',
              }}
            />
            ...
            {search && (
              <button onClick={() => { setSearch(''); loadData() }} style={{
                padding: '8px 12px', background: colors.border, color: colors.textSecondary, border: `1px solid ${colors.borderLight}`,
                borderRadius: 4, cursor: 'pointer', fontSize: 13,
              }}>Clear</button>
            )}
          </div>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 12, color: colors.textSecondary, whiteSpace: 'nowrap', flexShrink: 0 }}>Install from repo:</span>
            <input
              value={manualRepo}
              ...
              style={{
                flex: 1, padding: '6px 10px', background: colors.bgPrimary, border: `1px solid ${colors.borderLight}`,
                borderRadius: 4, color: colors.textPrimary, fontSize: 12, fontFamily: 'monospace', outline: 'none',
              }}
            />
            <button
              ...
              style={{
                padding: '6px 12px', background: colors.bgTertiary, color: colors.accent,
            ...
            <button
              ...
              style={{
                padding: '6px 12px', background: !manualRepo.trim() ? colors.borderLight : colors.accent,
                color: !manualRepo.trim() ? colors.textSecondary : colors.accentContrast,
          ...
          {/* Registry grid */}
          {loading ? (
            <div style={{ color: colors.inputPlaceholder, fontSize: 13, padding: 20 }}>Loading registry...</div>
          ) : registry.length === 0 ? (
            <div style={{ ...card, color: colors.inputPlaceholder, fontSize: 13 }}>
                  ...
                  <div key={skill.repo} style={{
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                        <div style={{ fontSize: 15, fontWeight: 600, color: colors.accent }}>{skill.name}</div>
                        {skill.downloads != null && (
                          <span style={{ fontSize: 11, color: colors.inputPlaceholder }}>{'\u2B50'} {skill.downloads}</span>
                        )}
                      </div>
                      <div style={{ fontSize: 12, color: colors.textSecondary, marginBottom: 8, lineHeight: 1.4 }}>
                        {skill.description || 'No description'}
                      </div>
                      <div style={{ fontSize: 11, color: colors.inputPlaceholder, marginBottom: 4 }}>
                        by <span style={{ color: colors.textSecondary }}>{skill.author}</span> &middot; {skill.repo}
                      </div>
                      {skill.tags.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 6 }}>
                          {skill.tags.slice(0, 5).map(t => (
                            <span key={t} style={{
                              padding: '2px 6px', background: colors.bgTertiary, color: colors.accent,
                              borderRadius: 3, fontSize: 10, border: `1px solid ${colors.bgActive}`,
                            }}>#{t}</span>
                          ))}
                        </div>
                      )}
                    ...
                    <div style={{ display: 'flex', gap: 6, marginTop: 12 }}>
                      <button onClick={() => doPreview(skill.repo)} disabled={previewLoading} style={{
                        padding: '5px 10px', background: colors.bgTertiary, color: colors.accent,
                        border: `1px solid ${colors.borderLight}`, borderRadius: 4, cursor: 'pointer', fontSize: 11,
                        flex: 1,
                      }}>Preview</button>
                      {isInstalled ? (
                        <span style={{ padding: '5px 10px', fontSize: 11, color: colors.success }}>Installed</span>
                      ) : (
                        <button onClick={() => doInstall(skill.repo)} disabled={installing === skill.repo} style={{
                          padding: '5px 10px', background: installing === skill.repo ? colors.borderLight : colors.accent,
                          color: installing === skill.repo ? colors.textSecondary : colors.accentContrast,
          ...
          {installed.length > 0 && (
            <div style={{ marginBottom: 16, textAlign: 'right' }}>
              <button onClick={doUpdateAll} disabled={updatingAll} style={{
                padding: '8px 16px', background: updatingAll ? colors.borderLight : colors.bgTertiary,
                color: updatingAll ? colors.textSecondary : colors.accent, border: `1px solid ${colors.borderLight}`,
              ...
              {installed.map(skill => (
                <div key={skill.name} style={{
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: colors.inputPlaceholder }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    <button onClick={() => doUpdate(skill.name)} disabled={updating === skill.name} style={{
                      padding: '5px 12px', background: colors.bgTertiary, color: colors.accent,
                      border: `1px solid ${colors.borderLight}`, borderRadius: 4, cursor: 'pointer', fontSize: 11,
                    }}>
